...
export default function CategoryPage() {
  ...
  const [activeCategory, setActiveCategory] = useState(slug || categories[0].slug);
        ...
        {categories.map((cat) => (
          ...
          >
            <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center ${
              activeCategory === cat.slug ? "bg-primary/10" : "bg-secondary/60"
            }`}>
              <img
            ...
            <span className={`text-[10px] sm:text-[11px] leading-tight line-clamp-2 font-medium ${
              activeCategory === cat.slug ? "text-primary font-bold" : "text-muted-foreground"
            }`}>
              {cat.name}
            </span>
          </Link>
