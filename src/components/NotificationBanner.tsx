...
const notifications = [
  { type: "deal", getMessage: () => { const p = getDiscountedProducts(); const item = p[Math.floor(Math.random() * p.length)]; return item ? `🔥 Price drop on ${item.name}! Now ₹${(item.discountPrice || item.price).toLocaleString()}` : null; } },
